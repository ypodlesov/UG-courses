...
1. `Max-Heapify` - служит для поддержки свойства пирамиды, выполняется за 
$O(\lg n)$.
...
Эта процедура всегда выбирает $A[r]$ как опорный. Разбиение подмассива $A[p \ldots r]$ будет выполняться относительно этого элемента.
